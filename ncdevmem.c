...
static struct memory_buffer *amdgpu_dmabuf_alloc(size_t size)
{
	struct memory_buffer *mbctx;
	struct ctx *ctx;

	mbctx = malloc(sizeof(*mbctx));
	if (!mbctx)
...
static void amdgpu_dmabuf_free(struct memory_buffer *mbctx)
{
	close(mbctx->memfd);
	/* release amdgpu memory */
	free(mbctx->priv);
	free(mbctx);
}

static struct memory_provider amdgpu_memory_provider = {
	.alloc = amdgpu_dmabuf_alloc,
	.free = amdgpu_dmabuf_free,
};

static struct memory_provider *amdgpu_provider = &amdgpu_memory_provider;
		...
		for (cm = CMSG_FIRSTHDR(&msg); cm; cm = CMSG_NXTHDR(&msg, cm)) {
			...
			if (do_validation)
				...
				print_nonzero_bytes(tmp_mem,
						    dmabuf_cmsg->frag_size);
#else
			dispatch_validation((void *)mem->priv,
					    dmabuf_cmsg->frag_offset,
					    dmabuf_cmsg->frag_size);
#endif
...
int main(int argc, char *argv[])
{
	...
	mem = amdgpu_provider->alloc(getpagesize() * NUM_PAGES);
	ret = is_server ? do_server(mem) : 1;
	amdgpu_provider->free(mem);
